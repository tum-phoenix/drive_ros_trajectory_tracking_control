...
//    ROS_INFO_NAMED(stream_name_,"phi y: %.5f, %.5f ", phi_soll, y_soll);
    ...
    for(int i=0; i<horizon_length; i++){
        ...
        v_soll[i] = target_points[i].twist.x;
    }


    const int STATES_ = 3; //number of states (y and phi)
    const int CONTROLS_ = 3; //number of control inputs (steering_front and steering_rear)
    ...
    B = 0, 0, 0, cycle_t_*v/l, -cycle_t_*v/l, 0, 0, 0, cycle_t_;
    ...
    Q = weight_y_, weight_phi_, weight_v_;

    dlib::matrix<double,STATES_,1> R; //Gewichtung der Stellgroessen
    R = weight_steeringFront_, weight_steeringRear_, weight_acc_;


    dlib::mpc<STATES_,CONTROLS_,horizon_length> controller(A,B,C,Q,R,lower,upper); //4*T ist der Zeithorizont fuer die praediktion, d.h. 4 Zeitschritte wird in die Zukunft simuliert

    dlib::matrix<double,STATES_,1> target[horizon_length];
    for(int i=0; i<horizon_length; i++){
        target[i]=y_soll[i], phi_soll[i], v_soll[i];
...
//    controller.set_target(target);
    ...
    double v_desired = v + action(2,0) * cycle_t_;
    ...
    ROS_INFO_NAMED(stream_name_, "velocity cmd = %.5f", drive_command_msg.lin_vel);
