...
PIDController::PIDController(ros::NodeHandle nh, ros::NodeHandle pnh):
    ...
    pnh_.getParam("pid/k_p_steer",k_p);
    pnh_.getParam("pid/k_i_steer",k_i);
    pnh_.getParam("pid/k_d_steer",k_d);
    pnh_.getParam("pid/k_p_vel", k_p_vel);
    pnh_.getParam("pid/k_i_vel", k_i_vel);
    pnh_.getParam("pid/k_d_vel", k_d_vel);
    esum=0;
    eold=0;
}
...
void PIDController::trajectoryCB(const drive_ros_msgs::TrajectoryConstPtr &msg) {
    ...
    double distance_to_trajectory_point = 6 * v * cycle_t_;
    ...
    double a_desired = k_p_vel * e_vel + k_i_vel * e_sum_vel + k_d_vel * (e_vel - e_old_vel) / cycle_t_;
    e_old_vel = e_vel;
    double v_desired = v + a_desired * cycle_t_;
    if (v_desired>5){
        v_desired=5;
    }
    if(v_desired<-5){
    ...
    ROS_INFO_NAMED(stream_name_, "v = %f", v);
